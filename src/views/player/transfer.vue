...
import { type PlusColumn, PlusSearch, FormChangeCallBackParams  } from "plus-pro-components";
...
const searchColumns: PlusColumn[] = [
  {
    ...
    prop: "id",
    valueType: "copy",
    fieldProps: computed(() => ({ placeholder: t('player.transfer.idPlaceholder') }))
  },
  {
    label: "用户名",
    ...
    prop: "name",
    valueType: "copy",
    fieldProps: computed(() => ({ placeholder: t('player.transfer.namePlaceholder') }))
  },
  {
    label: "用户ID",
    ...
    prop: "userId",
    valueType: "copy",
    fieldProps: computed(() => ({ placeholder: t('player.transfer.userIdPlaceholder') }))
  },
  {
    label: "状态",
    ...
    prop: "status",
    valueType: "select",
    fieldProps: computed(() => ({ placeholder: t('player.transfer.statusPlaceholder') })),
    options: [
      {
        label: "全部",
...
// 状态切换处理
const handleStatusChange = async (params: { row: TableRow; prop: string; value: any;}) => {
  ...
  try {
    ...
    if (res.success) {
      // 显示成功提示
      message(isDisabling ? "玩家已禁用" : "玩家已恢复正常", { type: "success" });
      // 可选：刷新列表数据
      // getList();
    } else {
      // 失败时恢复原状态
...
const handleEdited = (params: { row: TableRow; prop: string; value: any; oldValue: any }) => {
...
const generateMockData = (): TableRow[] => {
  ...
  const names = ['Slew', 'John', 'Alice', 'Bob', 'Charlie', 'David', 'Emma', 'Frank', 'Grace', 'Henry'];
  const merchants = ['abc123', 'xyz789', 'merchant001', 'merchant002', 'merchant003'];
  ...
  return Array.from({ length: 100 }).map((_, index) => {
    ...
    return {
      ...
      loginTime: randomDate.toLocaleString('zh-CN', { 
        year: 'numeric', 
        month: '2-digit', 
        day: '2-digit',
        hour: '2-digit',
      ...
      registerTime: randomDate.toLocaleString('zh-CN', { 
        year: 'numeric', 
        month: '2-digit', 
        day: '2-digit',
        hour: '2-digit',
...
const getList = async () => {
  ...
  try {
    ...
    // 注册时间范围筛选
    if (state.value.registerTime && Array.isArray(state.value.registerTime) && state.value.registerTime.length === 2) {
...
<template>
  <!-- 搜索表单 -->
    <el-card class="search-card" shadow="never" style="margin: 20px 20px 20px 20px;">
      <PlusSearch
      v-model="state"
  ...
  <!-- 表格 -->
  <el-card class="table-card" shadow="never" style="margin: 20px 20px 20px 20px;">
    <PlusTable
      v-loading="loading"
      :columns="tableConfig"
      :table-data="tableData"
      :is-selection="true"
      :action-bar="{ buttons, width: '220px', label: t('player.transfer.action') }"
      height="550px"
